    ...
    if [ -f "$orphan_rom" ]; then
        if [ -f "$GTT_LIST" ]; then
            if grep -q "|$orphan_rom|" "$GTT_LIST"; then
                ...
                display_name="[${time_display}] $game_name"
                echo "$display_name|$orphan_rom|PSP|1|$session_time|$time_display|launch|$session_time" >> "$temp_file"
...
if [ -f "$ROM" ]; then
    if [ -f "$GTT_LIST" ]; then
        if grep -q "|$ROM|" "$GTT_LIST"; then
            ...
            echo "$header_line" > "$temp_file"
            display_name="[${time_display}] $game_name"
            echo "$display_name|$ROM|PSP|1|$session_time|$time_display|launch|$session_time" >> "$temp_file"
            tail -n +2 "$GTT_LIST" >> "$temp_file"
            mv "$temp_file" "$GTT_LIST"
        fi
        temp_sorted="/tmp/gtt_sorted.txt"
        ...
        display_name="[${time_display}] $game_name"
        echo "$display_name|$ROM|PSP|1|$session_time|$time_display|launch|$session_time" >> "$GTT_LIST"
    fi
fi
