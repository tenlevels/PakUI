...
if [ -f "$GTT_LIST" ] && [ "$(grep -c "|launch" "$GTT_LIST")" -gt 2 ]; then
    ...
    if [ -f "$game_path" ]; then
        if [ "$game_plays" -lt 3 ]; then
            mins=$((RANDOM % 10 + 5))
            echo "Rediscover $game_name for $mins minutes|gtt|$game_path|$mins" >> /tmp/mission_types.txt
        else
            mins=$((RANDOM % 15 + 10))
            echo "Play more $game_name for $mins minutes|gtt|$game_path|$mins" >> /tmp/mission_types.txt
        fi
